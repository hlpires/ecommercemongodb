...
const Cart = ({ open, onClose, cartProps, onClear, removeItem }) => {
  ...
  return (
    ...
    <div className='modalCart' style={{ zIndex: '99999' }}>
      <div className='maaa' style={{ zIndex: '99999' }} onClick={onClose}></div>
      <div className='cart' style={{ ...mystyle, zIndex: '99999' }} >
        <div className='cartTittleBox'>
          <ShoppingCartCheckoutIcon style={{ fontSize: '46px', marginTop: '15px', marginLeft: '30px' }} />
          <h2>Carrinho</h2>
                ...
                <div>
                  {cartItens.slice(0, 4).map(({ name, price, imageurl, numero, id }) => (
                    <div key={'1'} className='cartItemBox'>
                      ...
                      <CloseIcon style={{ fontSize: '40px', marginTop: '40px', fontWeight: 'bold', cursor: 'pointer' }} onClick={() => removeItem(id)}></CloseIcon>
                    </div>
                  ))}
